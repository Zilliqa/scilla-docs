...
\begin{table}[!hbt]
	\caption{Gas for typechecking of expressions. \label{tab:typechecking} }
\centering
	\begin{tabular}{|p{3.5cm}|p{6.7cm}|p{5.5cm}|}
	\hline
	\textbf{Type of e}  & \textbf{$typGas(\tau)$} & \textbf{Remarks} \\ \hline
  Primitive type (
	\textbf{\texttt{\textcolor{teal}{String}}},
  \textbf{\texttt{\textcolor{teal}{IntX}}},
  \textbf{\texttt{\textcolor{teal}{BNum}}},
  \textbf{\texttt{\textcolor{teal}{ByStrX}}},
  \textbf{\texttt{\textcolor{teal}{Messsage}}},
  etc.),
  & \multirow{4}{5cm}{ $1$ } &
  \\ \hline
  \textbf{\texttt{\textcolor{teal}{ADT n $\tau_1$\dots $\tau_k$}}}
  &
  $ 1 + \sum_{i \in 1...k} typGas(\tau_{i}) $ &
  An abstract datatype such as \textbf{\textcolor{magenta}{\texttt{Pair}}}
  may be parameterised with other types $\tau_1$\dots $\tau_k$.
  Substitution needs to happen inside those types as well.
  \\ \hline
  \textbf{\texttt{\textcolor{teal}{MapType~$\tau_{key}~\tau_{val}$}}}
  &
  $ 1 + typGas(\tau_{key}) + typGas(\tau_{val}) $
  &
  \\ \hline
  \textbf{\texttt{\textcolor{teal}{FunType~$\tau_{arg}~\tau_{body}$}}}
  &
  $ 1 + typGas(\tau_{arg}) + typGas(\tau_{body}) $
  &
  \\ \hline
	\textbf{\texttt{\textcolor{teal}{PolyFun~$\alpha'~\tau_{body}$}}}
  &
  $ 1 + typGas(\tau_{body})$
  &
  The typechecker ensures that $\alpha'$ is different from
  $\alpha$, so that no unwanted variable capture takes place.
  \\ \hline
	\textbf{\texttt{\textcolor{teal}{TypeVar~$\alpha'$}}}
  &
  $$ 
	  \begin{cases}
      |\tau|, & \text{if } \alpha=\alpha', \\
      $1$, & \text{else } \\
    \end{cases}
  $$
    &
  $|\tau|$ is the size of the type being substituted, which is
    calculated using the same table, except that the size of a
    \textbf{\texttt{\textcolor{teal}{TypeVar}}} is always $1$.
  \\ \hline
	\end{tabular}
\end{table}

\section{Transition Invocation}
When a transition is invoked on a deployed contract by an end user (as
a part of a transaction), the user supplies a JSON file containing
information on which transition needs to be invoked and the parameters
to pass. Gas associated with such transition calls is equal to the
size of this JSON file.
